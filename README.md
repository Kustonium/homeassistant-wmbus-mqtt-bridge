...
### ⚙️ Uwaga o procesie deweloperskim

Ten projekt jest rozwijany z asystą AI (**Claude** od Anthropic) jako głównym narzędziem programistycznym. Maintainerem oraz osobą testującą każdą zmianę (human-in-the-loop) jest **Kustonium**.

Takie podejście jest świadome i transparentne — rozwój open source z asystą AI to ważna i rosnąca metodyka. Cały kod, każdy commit i każda decyzja architektoniczna są weryfikowane przez człowieka przed wypchnięciem na main.

### 🌍 Uwaga o tłumaczeniach

Tłumaczenia interfejsu webui na **niemiecki (DE), czeski (CS) i słowacki (SK)** zostały wygenerowane przez AI i mogą zawierać błędy lub nienaturalne sformułowania. **Polski (PL) i angielski (EN)** są utrzymywane ręcznie.

Native speakerzy są mile widziani — poprawki proszę zgłaszać przez [GitHub Issues](https://github.com/Kustonium/homeassistant-wmbus-mqtt-bridge/issues) lub Pull Requesty. Wszystkie tłumaczenia żyją w jednym pliku: [`rootfs/usr/bin/i18n.py`](rootfs/usr/bin/i18n.py).
...
This project is developed with AI assistance (**Claude** by Anthropic) as the primary coding tool. The maintainer and human-in-the-loop tester is **Kustonium**.

This approach is intentional and transparent — AI-assisted open source development is a valid and growing methodology. All code, every commit and every architectural decision is reviewed by a human before being pushed to main.

### 🌍 Translations notice

UI translations to **German (DE), Czech (CS) and Slovak (SK)** were generated by AI and may contain errors or unnatural phrasing. **Polish (PL) and English (EN)** are maintained manually.

Native speakers are welcome to submit corrections via [GitHub Issues](https://github.com/Kustonium/homeassistant-wmbus-mqtt-bridge/issues) or Pull Requests. All translations live in a single file: [`rootfs/usr/bin/i18n.py`](rootfs/usr/bin/i18n.py).
